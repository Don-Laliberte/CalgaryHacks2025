...
const QuizItem = ({ question, onClose, onNext, isLastQuestion }) => {
  ...
  const [highlightedOption, setHighlightedOption] = useState(0); // Track highlighted option index
  ...
  // Reset highlighted option when question changes
  useEffect(() => {
  ...
  const handleAnswerSelect = async (answer) => {
    ...
    // Control LEDs if Phidget is connected
    if (isConnected) {
      // Reset both LEDs
